    ...
    <template v-else>
      ...
      <label class="app-field">
        ...
        <input
          :value="editTags"
          placeholder="工作, 学习"
          @input="emit('update:editTags', ($event.target as HTMLInputElement).value)"
        />
      </label>
      ...
      <section class="reminders">
        ...
        <ul v-else class="reminder-list">
          <li v-for="item in reminders" :key="item.id">
            ...
            <div v-if="!item.enabled && item.snoozeCount < 3" class="snooze-btns">
              <button
                v-for="min in SNOOZE_OPTIONS"
                :key="min"
                class="link"
                type="button"
                @click="emit('snoozeReminder', item.id, min)"
              >
                +{{ min }}分
              </button>
            </div>
