...
**主窗 IA**：顶栏「今天 | 库」+ 跨库搜索；今天为焦点台分组列表（`list_focus_board` → 逾期 / 进行中 / 今日到期，互斥）；库为全部/已完成/标签/归档/回收站（`list_workbench_todos`）。选中后右侧滑入 Inspector（无选中不占列）。冷启动：`list.default_view` 为 today 或非法/doing/overdue/tag → 焦点台；all/done/archived/trash → 库对应归类。保存默认落点不切换当前页。Inspector autosave（切选/Esc 前 flush；`saveStatus`：idle 不展示 / dirty / saving / saved）；`get_todo` 含软删记录供回收站 Inspector，update/transition 仍拒软删。助理预览只读（标题/描述），无 dirty flush。搜索 Enter 走 `list_todos` keyword（Todo/Doing/Done）。焦点台行内快加默认今日截止。
