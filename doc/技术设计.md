...
**主窗 IA（v1.3）**：左视图导航（主归类 今日→进行中→逾期→已完成→全部；标签区；底栏归档/回收站）+ 中列表 + 右 Inspector。冷启动按 settings `list.default_view` 落点（出厂 today）。`useTodos` 调用 `list_workbench_todos(ListWorkbenchQuery)`（视图语义在后端；回收站走 `view=trash`）。Inspector autosave（切选/Esc 前 flush；`saveStatus`：idle / dirty / saving / saved）；`get_todo` 含软删记录供回收站 Inspector，update/transition 仍拒软删。助理预览只读（标题/描述），无 dirty flush。
...
预置 key：companion.*、main_window.bounds、autostart.enabled、list.default_sort、`list.default_view`（默认 `today`；合法值 today/doing/overdue/done/all/archived/trash，非法与 `tag` 回落 today）、notification.enabled、`ui.theme`（默认 system）、`ui.locale`（默认 zh-CN）

`update_settings`：先写 SQLite；仅当 `autostart.enabled` **相对当前值变化** 时再调 `tauri-plugin-autostart`。OS 同步失败（开发态路径、Windows 无启动项时报 os error 2 等）只打日志，**不回滚已保存偏好**、不阻断其它设置项。`ui.theme` / `ui.locale` 变更经 `settings-updated` 同步三窗外观。`list.default_view` 仅影响下次主窗冷启动落点，不因 `settings-updated` 切换当前侧栏；写入 `tag` 时回落为 `today` 再落库。
