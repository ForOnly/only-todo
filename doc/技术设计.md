...
**主窗 IA（v1.3）**：左视图导航 + 中列表 + 右 Inspector。`useTodos` 按 `WorkbenchView` 映射 `list_todos` 查询（含 `includeDeleted` 回收站）。Inspector autosave（切选/Esc 前 flush）；`get_todo` 含软删记录供回收站 Inspector，update/transition 仍拒软删。伴侣详情保持轻量壳。
