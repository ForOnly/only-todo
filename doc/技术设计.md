...
**桌面交互壳**：`installDesktopGuards`（三窗）禁用浏览器右键与文件拖入默认；`AppContextMenu` 提供剪切/复制/粘贴/全选。任务删除走 `useAppConfirm` / `AppConfirmDialog`（非 `window.confirm`）。表单控件：`AppSelect`、`AppDateTimePicker`（对外值仍为本地 `YYYY-MM-DDTHH:mm` 字符串，经 `date.ts` 与 ISO 互转；时分为两位文本输入）；打开态 Esc 在 capture 阶段 `stopImmediatePropagation`，不连带关父 `AppModal`；外层滚动关闭 Teleport 浮层（浮层自身滚动除外）。宿主见 `AppShellOverlays`。
