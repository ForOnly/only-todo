...
**主窗 IA（v1.3）**：左视图导航（主归类 今日→进行中→逾期→已完成→全部；标签区；底栏归档/回收站）+ 中列表 + 右 Inspector。冷启动按 settings `list.default_view` 落点（出厂 today）。`useTodos` 调用 `list_workbench_todos(ListWorkbenchQuery)`（视图语义在后端；回收站走 `view=trash`）。Inspector autosave（切选/Esc 前 flush；`saveStatus`：idle / dirty / saving / saved）；`get_todo` 含软删记录供回收站 Inspector，update/transition 仍拒软删。助理详情保持轻量壳；关详情 / 最小化 / 隐藏前若 dirty 则 flush。
