...
**几何**：工作区 `work_area`。自由态近缘吸附仍 ≤ 24px。贴边拖拽结束用被拖窗口 outer，阈值 `max(24, chrome.width + 8)`：近当前边只更新 `dock_y`；近对侧改 `dock_edge`；两侧都远 `undock_to_home`。点球出板：w/h 用上次面板，x/y 写入运行时 `temp_body_bounds`，不写 DB 的 x/y。条视觉 16px（HWND 可更宽），`float-chrome` `resizable: false`；apply 后按实际 outer 收进工作区，body 按实际条宽让位。拖离回球：松手中心 − 32，再按 64 回推进屏。程序化 apply 约 80ms `applying`。出面板先藏 chrome；回球/收条先藏 body。拖拽 settle 180ms；OS 左键仍按下则重臂。托盘左键：可见但飞出工作区视为丢失，走 show+修复。
