...
| companion_minimize | Esc 等：按矩阵回球或收条 |
| companion_collapse_to_strip | 面板 ×：一律收成贴边条 |
...
运行时 `RuntimeState.temp_body_bounds`：圆球家临时板本次位置。`companion_collapse_to_strip` / 隐藏助理 / 托盘再显示回球时清空。设置球→面板且临时板开着时提交进 `panel_bounds`。
