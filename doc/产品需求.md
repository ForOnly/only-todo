...
单 `float` 窗口：设置「默认形态」为主形态（`ball` 或 `panel`）；`docked` 为两种主形态都可进入的贴边态。

| 形态 | 交互 |
|------|------|
| **主形态 ball** | 显示圆球（窗 64×64，内 56 圆）；单击 → 临时 panel；panel 的 × / − 回到圆球（不藏托盘）；可拖动、可挂边 |
| **主形态 panel** | 只显示面板（320×480）；无「收起为圆球」；× → 隐藏至托盘；顶栏可拖（5px 阈值）、可挂边 |
| **docked** | 贴工作区左/右缘；视觉约 8px 蓝条，HWND 48×64（与圆球同高）且在屏内；**悬停展开**完整 panel；移开约 300ms 收回竖线；取消挂边 / 拖离 → **回到主形态** |

通用：拖拽至左右屏幕边缘（< 24px）自动挂边；「+」快速创建；双击条目 → 主窗口定位。圆球藏窗走托盘。

重启：若上次为挂边则恢复挂边，否则按主形态显示（临时展开的 panel 不跨隐藏记住）。设置「默认形态」保存后，未挂边时立即应用；挂边中只更新主形态、不收竖线。
...
|--------|------|
| 通知开关 / 默认排序 | v1.0 |
| 悬浮窗置顶 / 显示条数 / 默认形态（ball \| panel） | v1.1 |
| 开机自启 | v1.2 |
...
| AC-04 | 双击打开主窗口并定位 |
| AC-F01 | 主形态 ball：圆球显示 Todo+Doing 计数；单击展开 panel |
| AC-F02 | 主形态 ball：panel 点击「收起」或 × 回到圆球 |
| AC-F03 | 拖拽 panel/ball 至屏幕边缘自动挂边 |
| AC-F04 | 两种主形态均可挂边；悬停展开，移开收起；取消挂边回到主形态 |
| AC-F05 | 主形态 panel：只显示面板、无收起圆球；× 进托盘；重启按主形态（挂边除外） |
| AC-F06 | 有逾期任务时圆球高亮 |
